      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative  max-w-[80vw]">
        <div className="text-center mb-16 animate-fade-in">
          <Badge className="mb-4 bg-gradient-to-r from-green-600 to-emerald-600 text-white animate-pulse">
            <Leaf className="w-12 h-14 mr-1 animate-bounce" />
            Upcoming Phase
          </Badge>
        ...
        {/* Phase Timeline */}
        {/* Progress Bar */}
        {/* <div className="relative w-80vw h-2 rounded-full bg-gray-200 mb-8 overflow-hidden">
          <div className="relative h-2 w-full">
            <div className="absolute top-1/2 left-0 right-0 h-1 bg-gray-200 rounded-full -translate-y-1/2 z-0"></div>
            <div
              className="absolute top-1/2 left-0 h-1 bg-gradient-to-r from-green-500 to-emerald-500 rounded-full -translate-y-1/2 z-10 transition-all duration-700"
              style={{
                width: `${(phaseTimeline.findIndex(p => p.status === 'Current') + 1) / phaseTimeline.length * 100}%`,
              }}
            ></div>
          </div>
        </div> */}
        ...
        <div className="relative grid grid-cols-1 md:grid-cols-4 gap-6 mb-16">
          {/* Horizontal connecting line for medium+ screens */}
          <div className="hidden absolute top-24 h-3 bg-gray-200 z-0"></div>

          {/* Progress bar on top of the background line */}
          <div
            className="hidden md:block absolute top-24 left-0 h-3 bg-gradient-to-r from-green-400 to-emerald-500 z-10 transition-all duration-500"
            style={{
              width: `${(phaseTimeline.filter(p => p.status === 'Completed' || p.status === 'Current').length / phaseTimeline.length) * 100}%`
            }}
          ></div>
          ...
          {phaseTimeline.map((item, index) => (
            <div
              key={index}
              className={`relative text-center group animate-scale-in z-10 ${item.status === 'Current' ? 'scale-110' : ''
                }`}
              style={{ animationDelay: `${index * 150}ms` }}
            >
              {/* Card wrapper with background for completed and current */}
              <div
                className={`
          w-[15vw] rounded-md p-4 shadow-md transition-all duration-500
        ...
        `}
              >
                {/* Icon circle */}
                <div
                  className={`w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-3 transition-all duration-500 shadow-lg 
            ...
            >
              <div className="relative h-48 overflow-hidden">
                ...
                <div className="absolute top-4 left-4">
                  <Badge className="bg-white/90 h-6 w-30 text-sm text-gray-800 backdrop-blur-sm">
                    {product.phase}
                  </Badge>
                </div>
              ...
              <CardContent className="p-6">
                ...
                <div className="flex items-center justify-between">
                  <Badge variant="outline" className="text-green-600 text-base border-green-600">
                    {product.impact}
                  </Badge>
